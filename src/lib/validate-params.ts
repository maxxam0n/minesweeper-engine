...
export const isValidGameParams = (params: GameParams): boolean => {
	...
	if (!Number.isFinite(cols) || !Number.isFinite(rows) || !Number.isFinite(mines)) return false
	if (!Number.isInteger(cols) || !Number.isInteger(rows) || !Number.isInteger(mines)) return false
